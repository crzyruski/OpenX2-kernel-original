...
static unsigned int rsrcs_avail = 0;	/* resources available now*/
...
exit:
	...
	if (rsrcs_avail == 0) {
		...
		wait_event_interruptible(pmu_wait_q,
			(rsrcs_avail != 0));
		pr_debug("PRM resources are available again, wake up\n");
	}
...
int pmu_stop(struct pmu_results *pmu_results)
{
	...
	/* read results */
	if ((ret = pmu_read_register(ipm_client_id, PMU_CCNT, &ccnt))){
		pr_debug("pmu_read_register:PMU_CCNT failed\n");
		goto exit;
	}
	if ((ret = pmu_read_register(ipm_client_id, PMU_PMN0, &pmn0))){
		pr_debug("pmu_read_register:PMU_PMN0 failed\n");
		goto exit;
	}
	if ((ret = pmu_read_register(ipm_client_id, PMU_PMN1, &pmn1))){
		pr_debug("pmu_read_register:PMU_PMN1 failed\n");
		goto exit;
	}
	if ((ret = pmu_read_register(ipm_client_id, PMU_PMN2, &pmn2))){
		pr_debug("pmu_read_register:PMU_PMN2 failed\n");
		goto exit;
	}
	if ((ret = pmu_read_register(ipm_client_id, PMU_PMN3, &pmn3))){
		pr_debug("pmu_read_register:PMU_PMN3 failed\n");
		goto exit;
	}

	/* disable counting */
	if ((ret = pmu_disable_event_counting(ipm_client_id))){
		pr_debug("pmu_disable_event_counting failed\n");
		goto exit;
	}
