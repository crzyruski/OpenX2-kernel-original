...
/* relayfs interface */
static struct rchan *event_chan = NULL, *op_stats_chan = NULL;

/* Run/Idle state change, OPs change are all stored in event table. */
static struct event_type *event_table_head = NULL, *event_p = NULL;
/* Detail time cost on all OPs are stored in op_stats table */
static struct op_stats_type *op_stats_table_head = NULL, *op_stats_p = NULL;
...
static int dvfm_timeslot_init = 0;
...
static ssize_t duty_cycle_show(struct sys_device *sys_dev, struct sysdev_attribute * attr, char *buf)
{
	int len, i;
...
static ssize_t ticks_show(struct sys_device *sys_dev, struct sysdev_attribute * attr, char *buf)
{
	...
	for (i = 0; i < (event_num / 2); i++) {
		len += sprintf(buf + len, "op%d, run ticks:%u, idle ticks:%u "
			"run second:%u, idle second:%u count:%u\n",
...
static ssize_t switch_time_show(struct sys_device *sys_dev, struct sysdev_attribute * attr, char *buf)
{
	int i, j, len, op_num;
	...
	for (i = 0; i < op_num; i++) {
		...
		for (j = 0; j < op_num; j++) {
			...
			dvfm_driver->ticks_to_usec(op_switch[i][j].last_tick),
			dvfm_driver->ticks_to_usec(op_switch[i][j].min_tick),
			dvfm_driver->ticks_to_usec(op_switch[i][j].max_tick));
		}
		len += sprintf(buf + len, "\n");
	}
	return len;
...
static ssize_t stats_store(struct sys_device *sys_dev, struct sysdev_attribute * attr, const char *buf,
				size_t len)
{
...
static void write_event_chan(void)
{
	...
	event_chan = relay_open("event_table", NULL,
			PAGE_SIZE, EVENT_PAGE_NUM, &event_cb, NULL);
	if (event_chan == NULL) {
...
static void write_stats_chan(void)
{
	...
	op_stats_chan = relay_open("op_stats_table", NULL,
			PAGE_SIZE, OP_STATS_PAGE_NUM, &event_cb, NULL);
...
int dvfm_add_timeslot(int op_idx, int cpu_state)
{
	...
	if (op_idx != op_stats_p->op_idx) {
		...
		size = sizeof(struct op_stats_type);
		if (op_stats_p >= (op_stats_table_head + (OP_STATS_ARRAY_SIZE
			- size) / size))
			op_stats_p = op_stats_table_head;
...
static int cap_init(void)
{
	...
	dvfm_add_timeslot(curop, CPU_STATE_RUN);
	dvfm_add_event(curop, CPU_STATE_RUN, curop, CPU_STATE_RUN);
