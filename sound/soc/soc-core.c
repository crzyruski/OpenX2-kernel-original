...
/* workaround for pop-noise at music stop stage in some machine */
#if defined(CONFIG_SND_SOC_MICCO) 
extern int dapm_update_bits(struct snd_soc_dapm_widget *widget);
extern int dapm_set_pga(struct snd_soc_dapm_widget *widget, int power);
static int enable_mono_noise_fix;
#define MICCO_MONO_WODGET "SPK PA"
static int soc_micco_mono_noise_fix(struct snd_soc_codec *codec, int power)
{	
	struct snd_soc_dapm_widget *w;

	list_for_each_entry(w, &codec->dapm_widgets, list) {
		if (strncmp(w->name, MICCO_MONO_WODGET, sizeof(MICCO_MONO_WODGET)) == 0) {
			if (!power && w->power) {
				enable_mono_noise_fix = 1;
			} else if (power && !w->power && enable_mono_noise_fix) {
				enable_mono_noise_fix = 0;
			} else
				return 0;
			...
			if(w->event) {
				if(power) /* open */
					w->event(w, NULL, SND_SOC_DAPM_POST_PMU);
				else /* close */
					w->event(w, NULL, SND_SOC_DAPM_PRE_PMD);
			}
		}
	}
...
#if defined(CONFIG_SND_SOC_MICCO) 
		soc_micco_mono_noise_fix(codec, 0);
#endif
...
static int soc_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if (codec_dai->pop_wait) {
		...
		if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
			...
			codec_dai->pop_wait = 0;
#if defined(CONFIG_SND_SOC_MICCO) 
			soc_micco_mono_noise_fix(codec, 1);
#endif
...
#ifdef CONFIG_PM
static int soc_suspend_bits;
int soc_enable_suspend(int bit, int enable)
{
	if (enable) {		
		soc_suspend_bits &= ~bit;
	} else {
		soc_suspend_bits |= bit;
	}
	return 0;
...
int soc_get_suspend_bits(void)
{
	return soc_suspend_bits;
}
...
static int soc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (soc_suspend_bits) {
		printk(KERN_DEBUG"soc_suspend_bits %d\n", soc_suspend_bits);
		return 0;
	}
