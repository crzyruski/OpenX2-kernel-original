 ...
 */
static inline size_t w1_f27_fix_count(loff_t off, size_t count, size_t size)
{
	if (off > size)
		return 0;
...
static ssize_t w1_f27_read_bin(struct kobject *kobj,
			       ...
			       char *buf, loff_t off, size_t count)
{
	...
	if (mutex_lock_interruptible(&sl->master->mutex)) {
		count = 0;
		goto out_dec;
	}
...
static ssize_t w1_f27_write_bin(struct kobject *kobj,
				struct bin_attribute *bin_attr,
				char *buf, loff_t off, size_t count)
{
	struct w1_slave *sl = kobj_to_w1_slave(kobj);
