...
/* cannot call msleep as we will call i2c at interupt mode, by frank */
...
static void i2c_pxa_try_restore(struct pxa_i2c *i2c, int err)
{
	unsigned long flags;
	printk("I2C bus err %d, i2c_pxa_try_restore\n", err);	

	clk_disable(i2c->clk);
	udelay(1);
	clk_enable(i2c->clk);
	udelay(100);
	i2c_pxa_reset(i2c); /* do a reset to restore i2c, add by frank */
}
...
static void i2c_pxa_scream_blue_murder(struct pxa_i2c *i2c, const char *why)
{
	...
	printk("i2c: error: %s at addr 0x%x\n", why, (i2c->msg)?i2c_pxa_addr_byte(i2c->msg):0x0);
...
static void i2c_pxa_reset(struct pxa_i2c *i2c)
{
	printk(KERN_DEBUG"Resetting I2C Controller Unit\n");
			       ...
			       struct i2c_msg *msg, int num)
{
	...
	if (i2c->fast_mode) /* 400K speed, 2.5*8 = 20us */
		timeout = 5000*num; /* if not enough, pls increase it */
...
/* if we get i2c_err_count to MAX_I2C_ERR_RETRY, it means fatal err at i2c bus,
    we must reboot system, add by frank
*/
#define MAX_I2C_ERR_RETRY (50)
			    ...
			    struct i2c_msg msgs[], int num)
{
 ...
 out:
	...
	if (ret < 0) {
		i2c_err_count++;
		if (i2c_err_count>MAX_I2C_ERR_RETRY)
			panic("MAX_I2C_ERR_RETRY\n");	
		i2c_pxa_try_restore(i2c, ret);
		i2c_pxa_scream_blue_murder(i2c, "pio_xfer");		
	} else {
		i2c_err_count = 0;
	}
	return ret;
...
static void i2c_pxa_irq_txempty(struct pxa_i2c *i2c, u32 isr)
{
 ...
 again:
	...
	if (isr & ISR_BED) {
		int ret = BUS_ERROR;
		//printk("i2c tx: ISR_BED was set\n");
		udelay(25); /* wait i2c stable, add by frank */
		...
		icr |= ICR_ALDIE | ICR_TB | ICR_STOP; /* send stop, add by frank */
	} else if (isr & ISR_RWM) {
		/*
...
static int i2c_pxa_probe(struct platform_device *dev)
{
	...
	i2c->adap.retries = 1; /* onlt try one */
