...
int smb380_set_mode(unsigned char mode) {
	...
	if (mode<4 && mode!=1 && p_smb380->mode != mode) {
    	...
    	comres += p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_WAKE_UP__REG, &data1, 1);
	  	...
	  	p_smb380->mode = mode;
		printk(KERN_DEBUG"smb380_set_mode %d\n", mode);
	} 
	return comres;
...
/** X-axis orientation data readout 
	\param *a_x pointer for orientation
*/
int smb380_read_orientation_x(short *a_x) 
{
	int comres;
	unsigned char data[2];
	short orientation;
	...
	*a_x = *a_x << (sizeof(short)*8-(SMB380_ACC_X_LSB__LEN+SMB380_ACC_X_MSB__LEN));
	*a_x = *a_x >> (sizeof(short)*8-(SMB380_ACC_X_LSB__LEN+SMB380_ACC_X_MSB__LEN));
	axis_to_orientation((*a_x)*2,&orientation);
	*a_x=orientation;
	return comres;
...
/** Y-axis orientation data readout 
	\param *a_y pointer for orientation
*/
int smb380_read_orientation_y(short *a_y) 
{
	int comres;
	unsigned char data[2];	
	short orientation;
	...
	*a_y = *a_y << (sizeof(short)*8-(SMB380_ACC_Y_LSB__LEN+SMB380_ACC_Y_MSB__LEN));
	*a_y = *a_y >> (sizeof(short)*8-(SMB380_ACC_Y_LSB__LEN+SMB380_ACC_Y_MSB__LEN));
	axis_to_orientation((*a_y)*2,&orientation);
	*a_y=orientation;	
	return comres;
}


/** Z-axis orientation data readout 
	\param *a_z pointer for orientation
*/
int smb380_read_orientation_z(short *a_z)
{
	int comres;
	unsigned char data[2];	
	short orientation;
	...
	*a_z = *a_z << (sizeof(short)*8-(SMB380_ACC_Z_LSB__LEN+SMB380_ACC_Z_MSB__LEN));
	*a_z = *a_z >> (sizeof(short)*8-(SMB380_ACC_Z_LSB__LEN+SMB380_ACC_Z_MSB__LEN));
	axis_to_orientation((*a_z)*2,&orientation);
	*a_z=orientation;	
	return comres;
}

/** X,Y and Z-axis orientation data readout 
	\param *acc pointer to \ref smb380acc_t structure for x,y,z data readout
	\note data will be read by multi-byte protocol into a 6 byte structure 
*/
int smb380_read_orientation_xyz(smb380acc_t * acc)
{
	int comres;
	unsigned char data[6];
	short orientation;
	...
	acc->x = acc->x << (sizeof(short)*8-(SMB380_ACC_X_LSB__LEN+SMB380_ACC_X_MSB__LEN));
	acc->x = acc->x >> (sizeof(short)*8-(SMB380_ACC_X_LSB__LEN+SMB380_ACC_X_MSB__LEN));
	axis_to_orientation(acc->x*2,&orientation);
	acc->x=orientation;	

	acc->y = SMB380_GET_BITSLICE(data[2],SMB380_ACC_Y_LSB) | (SMB380_GET_BITSLICE(data[3],SMB380_ACC_Y_MSB)<<SMB380_ACC_Y_LSB__LEN);
	acc->y = acc->y << (sizeof(short)*8-(SMB380_ACC_Y_LSB__LEN + SMB380_ACC_Y_MSB__LEN));
	acc->y = acc->y >> (sizeof(short)*8-(SMB380_ACC_Y_LSB__LEN + SMB380_ACC_Y_MSB__LEN));
	axis_to_orientation(acc->y*2,&orientation);
	acc->y=orientation;
	...
	acc->z = acc->z << (sizeof(short)*8-(SMB380_ACC_Z_LSB__LEN+SMB380_ACC_Z_MSB__LEN));
	acc->z = acc->z >> (sizeof(short)*8-(SMB380_ACC_Z_LSB__LEN+SMB380_ACC_Z_MSB__LEN));
	axis_to_orientation(acc->z*2,&orientation);
	acc->z=orientation;
