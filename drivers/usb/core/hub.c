...
int usb_port_resume(struct usb_device *udev)
{
 ...
 SuspendCleared:
    ...
    // Pull down the modem wakeup pin
    gpio_direction_output(MFP_PIN_GPIO94, GPIO_LEVEL_LOW);
	return status;
}
...
static void hub_events(void)
{
	...
	while (1) {
		...
		for (i = 1; i <= hub->descriptor->bNbrPorts; i++) {
            ...
            printk(KERN_INFO "Davis: port status 0x%x port change 0x%x\n",
                    portstatus, 
                    portchange);
			if (portchange & USB_PORT_STAT_C_CONNECTION) {
