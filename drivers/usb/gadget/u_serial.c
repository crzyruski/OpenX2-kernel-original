...
#define GS_PORT_NUM   0   //Davis: only export one serial port now.
/*
...
static void gs_rx_push(unsigned long _port)
{
	...
	while (!list_empty(queue)) {
		...
		if (req->actual) {
            ...
            pr_info("Davis: count %d size %d packet 0x%x \n",
                count, size, packet[0]);
			port->n_read = 0;
		}
recycle:
...
static int gs_open(struct tty_struct *tty, struct file *file)
{
    ...
    pr_info("Davis: gs_open port_num %d\n", port_num);
	...
	if (port->port_usb) {
		...
		pr_info("Davis: gs_open: start ttyGS%d\n", port->port_num);
		gs_start_io(port);
...
struct tty_driver *gs_tty_driver;
...
int __init gserial_setup(struct usb_gadget *g, unsigned count)
{
	...
	pr_info("Davis: %s: registered %d ttyGS* device%s\n", __func__,
			count, (count == 1) ? "" : "s");
...
extern void tty_port_relay_register_gs(struct tty_driver*);
extern int tty_port_relay_unregister_gs(struct tty_driver*);
