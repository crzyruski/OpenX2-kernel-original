...
static void dumpreg(void) 
{
    ...
    printk(KERN_INFO "--mfp reg value gpio77 0x%x, gpio78 0x%x, gpio79 0x%x, \
                    gpio80 0x%x, gpio81 0x%x, gpio82 0x%x\n",
                    pxa3xx_mfp_read(MFP_CFG_PIN(GPIO77_USB_P3_1)),
                    pxa3xx_mfp_read(MFP_CFG_PIN(GPIO78_USB_P3_2)),
                    pxa3xx_mfp_read(MFP_CFG_PIN(GPIO79_USB_P3_3)),
                    pxa3xx_mfp_read(MFP_CFG_PIN(GPIO80_USB_P3_4)),
                    pxa3xx_mfp_read(MFP_CFG_PIN(GPIO81_USB_P3_5)),
                    pxa3xx_mfp_read(MFP_CFG_PIN(GPIO82_USB_P3_6)));
...
#else
#define dumpreg() do {} while(0)
#endif
static void pxa27x_ohci_power_ti1105(void) 
{
    printk(KERN_INFO "Initialize the TI 1105 PHY\n");
    gpio_request(MFP_PIN_GPIO4_2, "USBH SUSPEND");
    pxa3xx_mfp_set_afds(MFP_PIN_GPIO4_2, MFP_AF0, MFP_DS04X);
    gpio_direction_output(MFP_PIN_GPIO4_2, GPIO_LEVEL_LOW);
...
static int pxa27x_start_hc(struct pxa27x_ohci *ohci, struct device *dev)
{
	...
	if (cpu_is_pxa3xx())
		uhchr = (__raw_readl(ohci->mmio_base + UHCHR) |
			UHCHR_PCPL | UHCHR_SSEP1 )
			& (~(UHCHR_SSEP2 | UHCHR_SSEP3 | UHCHR_PSPL | UHCHR_SSE));
	else
...
int usb_hcd_pxa27x_probe (const struct hc_driver *driver, struct platform_device *pdev)
{
    ...
    /* Power the Ti1105 usb host phy */
    pxa27x_ohci_power_ti1105();
