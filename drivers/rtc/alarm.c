...
static struct rtc_device *external_alarm_rtc_dev;
...
from_old_alarm_set:
	...
	case ANDROID_ALARM_SET_RTC:
		...
		rv = rtc_set_time(alarm_rtc_dev, &rtc_new_rtc_time);
			if (external_alarm_rtc_dev)
				rtc_set_time(external_alarm_rtc_dev, &rtc_new_rtc_time);
		spin_lock_irqsave(&alarm_slock, flags);
	...
	case ANDROID_ALARM_GET_TIME(0):
		...
		case ANDROID_ALARM_SET_EXT(0): { /* an real alarm, let we set external rtc */		
				struct rtc_wkalrm rtc_alarm;					
				if(copy_from_user(&new_alarm_time, (void __user *)arg, sizeof(new_alarm_time))) {
					rv = -EFAULT;
					goto err1;
				}
				rtc_time_to_tm(new_alarm_time.tv_sec, &rtc_alarm.time);								

				if (new_alarm_time.tv_sec == 0)
					rtc_alarm.enabled = 0;
				else
					rtc_alarm.enabled = 1;
				if (external_alarm_rtc_dev && 0==rtc_alarm.time.tm_sec)
					rv = rtc_set_alarm(external_alarm_rtc_dev, &rtc_alarm);				
				ANDROID_ALARM_DPRINTF(ANDROID_ALARM_PRINT_INFO, 
					"ANDROID_ALARM_SET_EXT %d %d\n", new_alarm_time.tv_sec, rv);
			}
			break;
				...
				struct class_interface *class_intf)
{
	...
	if (alarm_rtc_dev) {
		printk("Android alram: detect a external rtc\n");
		external_alarm_rtc_dev = rtc; /* grab a external rtc */
		err = 0;
		goto err1;
	}
