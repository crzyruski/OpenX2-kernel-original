...
int ov7660_get_framerate(u32 format, u32 width, u32 height, u32 *numerator, u32 *denominator)
{
	int ret = 0;
	switch (format) {
		case V4L2_PIX_FMT_YUV422P:
...
static char name[] = "Omnivision7660";
...
int ov7660_register(int id)
{
	...
	camera_functions->init = ov7660_init;     	
	camera_functions->deinit = ov7660_deinit;     
	camera_functions->set_capture_format = ov7660_set_capture_format; 	
	camera_functions->start_capture =	ov7660_start_capture;	 		
	camera_functions->stop_capture =	ov7660_stop_capture; 			
	camera_functions->sleep = ov7660_sleep;	
	camera_functions->wakeup = ov7660_wake;	
	camera_functions->read_8bit = ov7660_read_8bit;			
	camera_functions->write_8bit = ov7660_write_8bit;			
	...
	camera_functions->set_power_mode = ov7660_set_power_mode;			
	camera_functions->set_contrast =	ov7660_set_contrast;			
	camera_functions->set_whitebalance =	ov7660_set_white_balance;		
	camera_functions->set_exposure =	 ov7660_set_exposure;			
	camera_functions->set_zoom = NULL;

	camera_functions->get_framerate = ov7660_get_framerate;
	
	camera_functions->name = name;
	camera_functions->id = id;

	/*register 7660 as LOW resolution */
	...
	const struct i2c_device_id *_id)
{
	...
	pidh = i2c_read(client, OV7660_PID);
	pidl = i2c_read(client, OV7660_VER);
	if ((pidh != PID_OV76XX) || (pidl != PID_7660)){
		goto err;
	}

	if(ov7660_register(id)<0)
		goto err;

	g_client = client;

	printk("OV7660 detected.\n");
...
static int sensor_remove(struct i2c_client *client)
{
	...
	ov7660_unregister(pdata->id);
	return 0;
}

static const struct i2c_device_id sensor_ov7660_id[] = {
	{ "sensor_ov7660", 0 },
	{ }
};
