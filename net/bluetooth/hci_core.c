...
#include <linux/tty.h>
static struct ps_data  *g_data=NULL;
void inithcidate(void  *data)
{
	g_data=(struct ps_data  *)data;
}
EXPORT_SYMBOL(inithcidate);
void resethci()
{
	struct hci_uart *hu = NULL;
	if(g_data&&g_data->tty)
	{
		hu = (struct hci_uart *)g_data->tty->disc_data;
		hci_send_cmd(hu->hdev, HCI_OP_RESET, 0, NULL);
	}
}
EXPORT_SYMBOL(resethci);
/**/
