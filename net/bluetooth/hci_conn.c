...
/*alex.chen added*/
__u16 getnotusedhandle(struct hci_dev *hdev)
{
	struct hci_conn_hash *h = &hdev->conn_hash;
	
	__u16 handle=1;
	BT_DBG("hdev=%p,isempty=%d\n",hdev,list_empty(&h->list));
	for(handle=1;handle<65535;handle++)
	{
...
struct hci_conn *hci_conn_add(struct hci_dev *hdev, int type, bdaddr_t *dst)
{
	...
	conn->handle=getnotusedhandle(hdev);
